...
const post_handler = (req,res) => {
    ...
    if(path == '/formparameter'){
        ...
        req.on('data',(data)=>{console.log('request.on(data) =',data.length);});
        req.on('end',()=>{});
        res.end();
    }
    else {
...
const headersResponse = (req,res) => {
    ...
    res.setHeader('qwe-qwed','-qwe-qwe');
    res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8', 'qwe-qwe':'qwe-qwe'});

    res.end('request headers:' + JSON.stringify(req.headers) + 'response headers:' + JSON.stringify(res.getHeaders()));
}
...
const socketResponse = (req,res) => {
    ...
    res.end(
        ...
        `server ip: ${JSON.stringify(addresses)}\n` +
